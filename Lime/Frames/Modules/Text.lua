...
local fontString = lime:CreateFontString(nil, "BACKGROUND", "GameFontHighlightSmall")
...
function lime:UpdateFont()
	fontString:SetFont(LibStub("LibSharedMedia-3.0"):Fetch("font", lime.db.font.file), lime.db.font.size, lime.db.font.attribute)
...
local function getHealthText2(self)
	if not lime.db.units.showAbsorbHealth then
		return ""
	elseif lime.db.units.healthType == 1 then
		if lime.db.units.shortLostHealth then
...
local function checkRange(self)
	if lime.db.units.healthRange == 1 then
		return true
	elseif self.outRange then
		return lime.db.units.healthRange == 3
	else
		return lime.db.units.healthRange == 2
	end
end
...
function limeMember_UpdateLostHealth(self)
	if lime.db.units.nameEndl then
...
function limeMember_UpdateName(self)
	if self.unit ~= "player" and UnitIsUnit("player", self.unit) and self:GetParent().partyTag then
		self:GetParent().partyTag.tex:SetColorTexture(unpack(lime.db.partyTagParty))
		lime.playerRaidIndex = self.unit:match("raid(%d+)")
		if lime.playerRaidIndex then
			lime.playerRaidIndex = tonumber(lime.playerRaidIndex)
			prevPlayerGroup = lime.playerGroup
			lime.playerGroup = select(3, GetRaidRosterInfo(lime.playerRaidIndex))
			if prevPlayerGroup and prevPlayerGroup ~= lime.playerGroup then
				lime.headers[prevPlayerGroup].partyTag.tex:SetColorTexture(unpack(lime.db.partyTagRaid))
			end
		end
	end
...
function limeMember_UpdateNameColor(self)
	if lime.db.colors[self.class] and (self.optionTable.className or (self.isOffline and self.optionTable.offlineName) or (self.outRange and self.optionTable.outRangeName) or ((self.isGhost or self.isDead) and self.optionTable.deathName)) then 
	self.name:SetTextColor(lime.db.colors[self.class][1], lime.db.colors[self.class][2], lime.db.colors[self.class][3])
	else
